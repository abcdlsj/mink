...
5. 每个 Agent 在承载它的 Computer 上拥有按 canonical Agent ID 寻址的长期 Workspace，跨消息、Work 和重启保留。当前版本每个 Agent 只有一个 active Run，其余输入进入 Inbox 排队。Sandbox runtime 与 Run 临时状态有独立生命周期；trusted local provider 可直接使用 Workspace，但不能冒充强 Sandbox。跨 Agent/Computer 成果只通过显式 Artifact 交换。
...
12. UI/UX 是产品合同，不是实现后的装饰。对话优先，Work/Artifact 就地展开，Agent 队列与异常状态可理解、可操作；基础设施术语不得占据主界面。
13. `DESIGN.md` 是持续演进的产品设计真相。改变核心语义前先更新设计；不得为了复用旧代码而恢复旧概念或兼容路径。
14. 每次开发前必须先阅读仓库根目录的 `AGENTS.md` 和 `CLAUDE.md`，确认当前产品与工程约束后再读代码、设计或动手修改。
15. Go 代码遵循 Google Go Style Guide 与 Best Practices：包和标识符命名简短、准确、一致，正确主路径保持左对齐，错误尽早返回，相关逻辑清晰分块。优先复用稳定核心而不是复制，但只在真实重复或替换边界出现时抽象；禁止为了“通用”牺牲可读性、类型安全和主路径的直接性。
