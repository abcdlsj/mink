...
4. 主动控制上下文体积：只读取当前工作所需的代码、设计章节和验证输出；总结已确认事实，不在上下文中反复堆叠完整日志、重复文件内容或已经失效的探索结果。
...
- 每完成一个语义完整、验证通过且可独立提交的基线，立即更新 `GOAL.md` 并提交；随后生成非空 handoff 文档，并使用 `superset agents create` 在当前 Superset workspace 创建新的承接 session，不在原 session 继续累积下一阶段上下文。
- handoff 文档只记录当前事实、已完成内容、验证证据、未完成风险和唯一下一步；新 session 创建成功后删除已被承接的旧 handoff，避免过期交接稿成为事实来源。
