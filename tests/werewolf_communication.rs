...
const SHARED_GAME_CONTEXT: &str = concat!(
    ...
    "协作消息处理规则：收到消息后先检查结构化收件人、mention、reply 目标和 DM 成员关系，再决定是否需要回复；结构化路由优先于正文中的名字或语气。",
    "频道活动、没有路由给你的公开消息，以及其他成员之间的 reply，默认只是上下文，不是给你的任务；没有明确路由给你时保持等待，不要为了证明已看到而发言。",
    "不得代替其他成员确认收件、复述其行动、催促其补充，或用第一人称承接不属于你的待办；不得回答明确写给其他成员的问题。",
    "只有在消息通过 DM、结构化 mention、reply 目标或明确的结构化指派指向你时，才把它当作需要你处理的请求；发送前再次确认你的回复不会改变原消息的收件人。",
);
    ...
    if profile.is_god {
        ...
        format!(
            concat!(
                ...
                "公开频道中的旁观消息只用于建立上下文；如果消息是写给其他成员的，你不得确认、转述或催促对方，也不得把对方的待办写成自己的回复。",
