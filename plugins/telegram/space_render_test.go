...
func TestNewSpaceAgentRepliesSkipsPreExistingMessages(t *testing.T) {
	...
	if _, err := a.Spaces().AppendAgentMessage(sp.ID, space.PersonaInfo{ID: "coder", Display: "Coder"}, "old reply", "", nil, "", nil, nil); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := a.Spaces().AppendAgentMessage(sp.ID, space.PersonaInfo{ID: "coder", Display: "Coder"}, "fresh reply", "", nil, "", nil, nil); err != nil {
		t.Fatal(err)
	}
