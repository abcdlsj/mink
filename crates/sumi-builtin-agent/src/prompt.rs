//! Sumi-specific prompt contracts and assembly.

use sha2::{Digest, Sha256};

use sumi_agent_core::Message;
...
pub fn product_contract() -> String {
    concat!(
        ...
        "Create a Task with `sumi agent task create --title <title> --json` when you own work that needs long-term state tracking: work that spans multiple Runs, waits on other Members, or must stay visible to collaborators across time. A Task binds one Source Thread, which must be a Root Message, and may link other Threads whose context it needs; link them in the same step by repeating `--thread <thread-id>`. Create it when `context.task_id` is absent, using the current Focus as the Source Thread. If the current Run is already bound to a Task and the new work belongs to that Task, do not create a second Task. If the work is independent, first post a new Root Message to a shared Channel with `sumi agent message send --channel <channel-id> --body <text> --json`, then create the Task from the returned `thread_id` with `sumi agent task create --title <title> --source <thread-id> --json`, adding `--thread` for related context. Keep the Task current: post progress updates to the Source Thread, reference the Task as `!<seq>` in messages, and finish with `sumi agent task done --result-file <path> --json` (or `sumi agent task submit-review --body-file <path> --json` when review is required). Close it with `sumi agent task close --reason <reason> --json` when the work is no longer needed. Do not create a Task for a single-message reply, work finished within the same Run, or an ambient observation.\n",
...
pub fn codex_turn_instruction(encoded_view: &str) -> String {
    format!(
        "{}\n\n{}\n\n{}",
        product_contract(),
        driver_contract(),
...
pub fn driver_contract_hash() -> String {
...
/// Assemble the system messages for a Sumi builtin turn: cacheable stable text,
/// dynamic identity, and the builtin tool usage contract.
pub fn system_messages(identity: &str, role: &str) -> Vec<Message> {
    vec![
        Message::cacheable_system(format!("{}\n\n{}", product_contract(), driver_contract())),
