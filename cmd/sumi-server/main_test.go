...
func TestAuthCommandCreatesOneTimeBrowserURL(t *testing.T) {
	...
	if err := run([]string{"auth", "--server", server.URL, "--human-key-file", keyPath}, &stdout, &stderr); err != nil {
		t.Fatal(err)
	}
...
func TestAuthCommandFailsClosedOnCredentialFiles(t *testing.T) {
	...
	for name, path := range tests {
		t.Run(name, func(t *testing.T) {
			var stdout bytes.Buffer
			var stderr bytes.Buffer
			err := run([]string{"auth", "--human-key-file", path}, &stdout, &stderr)
			if err == nil {
...
func TestAuthCommandRejectsRedirectAndUnsafeHandoff(t *testing.T) {
	...
	for name, handler := range tests {
		t.Run(name, func(t *testing.T) {
			...
			var stdout bytes.Buffer
			var stderr bytes.Buffer
			err := run([]string{"auth", "--server", server.URL, "--human-key-file", keyPath}, &stdout, &stderr)
