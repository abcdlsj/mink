...
- `Agent graph` 是默认关闭的实验功能：只有 Settings 开启 Experimental features 后，Space rail（Network 图标）与移动端 Space tools 导航才显示入口；路由为 `/s/$spaceSlug/graph`，未开启时直达页面显示禁用说明与 Settings 链接。
