...
func TestBuildSystemPromptAddsMemoryProposalProtocol(t *testing.T) {
	...
	for _, want := range []string{
		...
		"authorization_text",
		"call propose_memory",
		"existing memory changes -> propose the intended update",
		"Scope rule: prefer current channel/conversation memory",
		"Forbidden: write_memory/update_memory/delete_memory",
		"!memory confirm <id>",
...
func TestBuildExternalPromptStopsFakeCapabilityInstructions(t *testing.T) {
	...
	for _, want := range []string{
		...
		"action:\"remember\"",
		"action:\"update\"",
		"JSON fence requirements: body must be a valid JSON string with escaped newlines",
		"Scope rule: prefer current channel/conversation memory for local facts",
		"No direct tool visible: do not list host tools like Bash/Edit/Read as Sumi product capabilities.",
		"If asked about remember_memory: say you cannot directly call the low-level tool",
		"If user says to use the Sumi-exposed memory way: produce a structured memory proposal",
		"Sumi memory action:",
