...
pub const DEFAULT_DRIVER_CONTRACT: &str = concat!(
    ...
    "workspace/<path> or memory/<path>; bare Memory paths like MEMORY.md default to memory/. ",
    "Shell writes are allowed under workspace/, runs/ ($TMPDIR), and /tmp; /tmp is scratch only.",
);
