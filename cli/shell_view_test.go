...
const testCLIChatSource = "cli:direct:20260716-direct-deadbeef"
...
func TestRenderHeaderMatchesCodexShell(t *testing.T) {
	m := newShellModel(context.Background(), fakeShellApp{}, testCLIChatSource)
	m.width = 96
	...
	if !strings.Contains(out, "model") || !strings.Contains(out, "cwd") || !strings.Contains(out, "direct-") {
		t.Fatalf("renderHeader() = %q, want chat facts", out)
	}
...
func TestRenderHeaderKeepsChatOnNarrowWidth(t *testing.T) {
	m := newShellModel(context.Background(), fakeShellApp{}, testCLIChatSource)
	m.width = 32

	out := ansi.Strip(m.renderHeader(m.state()))
	if !strings.Contains(out, "direct-") {
		t.Fatalf("renderHeader() = %q, want chat visible", out)
	}
...
func TestViewKeepsFullHeaderVisible(t *testing.T) {
	...
	for _, want := range []string{"Sumi", "direct-"} {
