...
func (s *State) SaveRuntimeSession(ctx context.Context, session RuntimeSession) error {
	if !validID(session.AgentID) || !validID(session.ComputerID) || session.PlacementDesiredRevision == 0 || session.Token == "" || session.ExpiresAt.IsZero() || session.UpdatedAt.IsZero() {
	...
	existing, found, err := readRuntimeSession(tx.QueryRowContext(ctx, `
		SELECT agent_id, computer_id, placement_desired_revision, token, expires_at, updated_at
	...
	if found {
		if session.PlacementDesiredRevision < existing.PlacementDesiredRevision {
			return errors.New("runtime session placement desired revision is stale")
		}
		if session.PlacementDesiredRevision == existing.PlacementDesiredRevision {
		...
		result, err := tx.ExecContext(ctx, `
			...
			SET computer_id = ?, placement_desired_revision = ?, token = ?, expires_at = ?, updated_at = ?
			WHERE agent_id = ? AND placement_desired_revision = ?
		`, session.ComputerID, session.PlacementDesiredRevision, session.Token, unixNano(session.ExpiresAt), unixNano(session.UpdatedAt), session.AgentID, existing.PlacementDesiredRevision)
		if err != nil {
		...
		INSERT INTO runtime_sessions(agent_id, computer_id, placement_desired_revision, token, expires_at, updated_at)
		VALUES(?, ?, ?, ?, ?, ?)
	`, session.AgentID, session.ComputerID, session.PlacementDesiredRevision, session.Token, unixNano(session.ExpiresAt), unixNano(session.UpdatedAt)); err != nil {
...
func (s *State) DeleteRuntimeSession(ctx context.Context, agentID, computerID string, placementDesiredRevision uint64) error {
	if !validID(agentID) || !validID(computerID) || placementDesiredRevision == 0 {
	...
	`, agentID, computerID, placementDesiredRevision); err != nil {
...
func (s *State) RuntimeSessions(ctx context.Context) ([]RuntimeSession, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&session.AgentID, &session.ComputerID, &session.PlacementDesiredRevision, &session.Token, &expiresAt, &updatedAt); err != nil {
...
func readRuntimeSession(row interface{ Scan(...any) error }) (RuntimeSession, bool, error) {
	var session RuntimeSession
	var expiresAt, updatedAt int64
	err := row.Scan(&session.AgentID, &session.ComputerID, &session.PlacementDesiredRevision, &session.Token, &expiresAt, &updatedAt)
