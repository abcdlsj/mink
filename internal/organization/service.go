...
func (s *Service) GetHuman(ctx context.Context, request *connect.Request[organizationv1.GetHumanRequest]) (*connect.Response[organizationv1.GetHumanResponse], error) {
	...
	id, err := id.CanonicalID(request.Msg.GetHumanId(), "human id")
	if err != nil {
		return nil, err
	}
	human, err := s.store.GetHuman(ctx, id)
	if errors.Is(err, organizationapp.ErrHumanNotFound) {
...
func (s *Service) SetHumanStatus(ctx context.Context, request *connect.Request[organizationv1.SetHumanStatusRequest]) (*connect.Response[organizationv1.SetHumanStatusResponse], error) {
	...
	humanID, err := id.CanonicalID(request.Msg.GetHumanId(), "human id")
	if err != nil {
		return nil, err
	}
	...
	human, err := s.store.SetHumanStatus(ctx, organizationapp.SetHumanStatusCommand{RequestID: requestID, Actor: subject, HumanID: humanID, Status: status, Now: s.now()})
	if errors.Is(err, authoritydomain.ErrPermissionDenied) {
	...
	if errors.Is(err, organizationapp.ErrHumanStatusConflict) {
...
func organizationMessage(organization organizationapp.Organization) *organizationv1.Organization {
...
func humanMessage(human organizationapp.Human) *organizationv1.Human {
