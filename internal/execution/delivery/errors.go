...
func serviceError(err error) error {
	...
	case errors.Is(err, executionapp.ErrDeliveryNotFound), errors.Is(err, executionapp.ErrRunNotFound),
		errors.Is(err, collaborationapp.ErrSpaceNotFound), errors.Is(err, collaborationapp.ErrThreadNotFound):
		return connect.NewError(connect.CodeNotFound, err)
	case errors.Is(err, executionapp.ErrInboxRequestConflict), errors.Is(err, executionapp.ErrRunCompletionConflict):
		return connect.NewError(connect.CodeAlreadyExists, err)
	case errors.Is(err, executionapp.ErrDeliveryNotAvailable), errors.Is(err, executionapp.ErrDeliveryCursorUnavailable),
		errors.Is(err, executionapp.ErrRunNotAccepted), errors.Is(err, executionapp.ErrRunNotRunning),
		errors.Is(err, executionapp.ErrRunLaunchActive), errors.Is(err, executionapp.ErrRunLaunchStale),
		errors.Is(err, executionapp.ErrRunLaunchExpired), errors.Is(err, executionapp.ErrInboxBasisMismatch),
		errors.Is(err, collaborationapp.ErrSpaceArchived):
		return connect.NewError(connect.CodeFailedPrecondition, err)
	case errors.Is(err, executionapp.ErrRunAlreadyActive):
		return connect.NewError(connect.CodeResourceExhausted, err)
	case errors.Is(err, executionapp.ErrInvalidDeliveryLimit), errors.Is(err, executionapp.ErrRunInvalidOutcome),
