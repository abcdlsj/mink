...
func TestBootstrapLocalAccountIsOneShotAndIssuesExistingBrowserSession(t *testing.T) {
	...
	bootstrap, err := database.EnsureAuthority(ctx, "bootstrap-credential-abcdefghijklmnopqrstuvwxyz-0123456789", now)
	if err != nil {
		t.Fatal(err)
	}
	required, err := database.LocalAccountSetupRequired(ctx)
	if err != nil || !required {
	...
	principal, err := database.BindBootstrapLocalAccount(ctx, command)
	if err != nil {
		t.Fatal(err)
	}
	...
	account, err := database.GetLocalAccount(ctx, "owner")
	if err != nil || account.Human != principal || account.Identity.Subject != "owner" {
		t.Fatalf("account = %+v, %v", account, err)
	}
	session, err := database.AuthenticateBrowserSession(ctx, command.SessionToken, now.Add(time.Minute))
	...
	if _, err := database.BindBootstrapLocalAccount(ctx, command); !errors.Is(err, ErrLocalAccountSetupDone) {
		t.Fatalf("second setup error = %v", err)
	}
	var identities, sessions, audits int
	if err := database.db.QueryRow(`SELECT count(*) FROM auth_identities WHERE provider = 'local'`).Scan(&identities); err != nil {
		t.Fatal(err)
	}
	if err := database.db.QueryRow(`SELECT count(*) FROM browser_sessions`).Scan(&sessions); err != nil {
		t.Fatal(err)
	}
	if err := database.db.QueryRow(`SELECT count(*) FROM audit_events WHERE action = ?`, AuditAuthIdentityBind).Scan(&audits); err != nil {
		t.Fatal(err)
	}
...
func TestLocalAccountAndSessionFailClosedWhenHumanIsDisabled(t *testing.T) {
	...
	_, err = database.BindBootstrapLocalAccount(ctx, authorityapp.BindBootstrapLocalAccountCommand{
		RequestID:      uuid.NewString(),
		BootstrapHuman: Principal{Kind: "human", ID: bootstrap.Human.ID, OrganizationID: bootstrap.Organization.ID},
		Identity:       authorityapp.AuthenticationIdentity{Provider: "local", Subject: "owner"},
		Password: authorityapp.PasswordDigest{
	...
	if _, err := database.db.Exec(`UPDATE humans SET status = 'disabled' WHERE id = ?`, bootstrap.Human.ID); err != nil {
		t.Fatal(err)
	}
	if _, err := database.GetLocalAccount(ctx, "owner"); !errors.Is(err, ErrPermissionDenied) {
		t.Fatalf("disabled local account error = %v", err)
	}
	if _, err := database.AuthenticateBrowserSession(ctx, token, now.Add(time.Minute)); !errors.Is(err, ErrPermissionDenied) {
		t.Fatalf("disabled session error = %v", err)
	}
	if err := database.CreateBrowserSession(ctx, authorityapp.CreateBrowserSessionCommand{
