...
Local Owner setup 是首次加入 Sumi 的 welcome flow。Username 必须明确显示服务端同一规则：3–32 位，只允许 ASCII 字母、数字、点、下划线和连字符；password 为 12–256 个 Unicode 字符。首个 Owner 仍必须提交一次性高熵证明，防止任意打开 fresh Server 的浏览器抢占 Owner；但该证明在产品语义中只称 **Owner setup code**，不向用户暴露 `bootstrap credential`、`human.key` 或 `owner.key` 等内部载体。它只用于这次 Owner 交接，不成为账户凭证，浏览器不显示、记录或持久化；Server 仍做权威校验，UI 不读取失败响应 body。

Computer 首次连接收口为一条用户命令：浏览器创建 10 分钟、一次性 pairing，将 `pairing.Bundle` v1 的 Server origin、Server identity、request ID、32-byte random pairing token 和 UTC expiry 编码为 `sumi-pair-v1.<base64url(JSON)>` connection code，然后只复制 `sumi computer start --pairing-code <code>`。CLI 必须先证明 current-user Sumi install 存在，未安装时在消费 code 前 fail closed；随后 strict decode 并重验 endpoint、identity、token、expiry 与版本，持久化 pairing attempt，消费 pairing，再启动已安装的 Computer service。Active install 会持久化其 canonical data root，所以用 `--data-root` 安装后的普通 service 命令也不需要重复该参数；旧 active manifest 缺少该字段时只回退既有默认。中途不确定可用同一 code 续传，不得为新 token 覆盖 durable attempt。

Connection code 是编码而非加密，安全性来自 pairing token 本身的高熵、10 分钟有效期和服务端一次消费。为换取一行命令，该 secret 会进入 shell history 与短暂的 process argv；这是显式接受的权衡，不得话术伪装为“不落痕”。Web 只长期渲染脱敏命令预览，完整 code 只在当次页面内存和用户主动 Copy 时进入剪贴板，不进入 URL、日志或 localStorage；CLI 成功和错误均不回显 code。下载 JSON 不再是 Web 主路径，旧 `pair join --file` 只作低层兼容能力保留。

浏览器只能为安全 endpoint 生成 connection code：literal loopback HTTP 使用 `literal-loopback`，系统可信 HTTPS 使用 `system-trust`；raw remote HTTP、hostname loopback HTTP、带 userinfo/path/query/fragment 的 origin 一律 fail closed，并给出改用 literal `127.0.0.1` 或可信 HTTPS 的操作说明。不得为了远程 demo 放宽 Server/Computer endpoint 合同。UI 必须明示本机先决条件是已安装 `sumi` CLI，并提供显式 Computer facts 刷新；注册完成与否仍以 Server 的 Computer 列表为准，不能用前端本地状态伪造成功。
